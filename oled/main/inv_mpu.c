...
//    unsigned char accel_intel;
    ...
    unsigned char s0_do;
    unsigned char s1_do;
    unsigned char i2c_delay_ms_ctrl;
...
#define BIT_S0_delay_ms_EN     (0x01)
#define BIT_S2_delay_ms_EN     (0x04)
...
int mpu_get_temperature(long *data, unsigned long *timestamp)
{
	...
	data[0] =
			(long) ((35
					+ ((raw - (float) st.hw->temp_offset) / st.hw->temp_sens))
					* 65536L);
	return 0;
}
...
static int get_accel_prod_shift(float *st_shift)
{
	...
	if (i2c_read(st.hw->addr, 0x0D, 4, tmp))
		return 0x07;
	//log_my("%s,%d:.|get_accel_prod_shift|\n", __FUNCTION__, __LINE__);
...
int mpu_run_self_test(long *gyro, long *accel)
{
	...
	//log_my("%s,%d:|accel_result = 0x%02x | gyro_result = 0x%02x \n", __FUNCTION__, __LINE__,accel_result,gyro_result);
	result = 0;
	if (!gyro_result)
		result |= 0x01;
	if (!accel_result)
...
int setup_compass(void)
{
    ...
    data[0] = 0x03;
    if (i2c_write(st.hw->addr, st.reg->i2c_delay_ms_ctrl, 1, data))
        return -1;
 ...
 *  \n To select a fractional wake-up frequency, round down the value passed to
 *  @e lpa_freq.
 *  \n The MPU6500 does not support a delay_ms parameter. If this function is used
...
uint8_t mpu_dmp_get(mpu_data_* mpu_data)
{
	...
	if (sensors & INV_WXYZ_QUAT)
	{
		...
		q3 = quat[3] / q30;
		//计算得到俯仰角/横滚角/航向角
		mpu_data -> pitch = asin(-2 * q1 * q3 + 2 * q0 * q2) * 57.3;	// pitch
		mpu_data -> roll = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2 * q2 + 1)
				* 57.3;	// roll
		mpu_data -> yaw = atan2(2 * (q1 * q2 + q0 * q3),
				q0 * q0 + q1 * q1 - q2 * q2 - q3 * q3) * 57.3;	//yaw
	}
	else
		return 2;

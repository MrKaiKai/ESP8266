...
int oled_write_cmd(uint8_t command)
{
	...
	ret = i2c_master_cmd_begin(I2C_OLED_MASTER_NUM, cmd,
 ...
 ****日  期：
 ******************************************************/
void OLED_ShowNum_F(uint8_t x, uint8_t y, uint32_t num, uint8_t len,
		uint8_t size2)
{
	uint8_t t, temp;
	uint8_t enshow = 0;
 ...
 ****日  期：2017-03-04
 ******************************************************/
void OLED_ShowFloatNum(uint8_t x, uint8_t y, float num, uint8_t IntLen,
		uint8_t FloatLen, uint8_t size)
{
	float Num = num - (float) ((uint16_t) num);
	uint16_t u = (uint16_t) ((float) Num * (float) oled_pow(10, FloatLen))
			% oled_pow(10, FloatLen);
	OLED_ShowNum(x, y, (uint16_t) num, IntLen, size);
...
//日  期:
/*******************************************/
void OLED_Float(uint8_t x, uint8_t y, float num, uint8_t IntLen,
		uint8_t FloatLen, uint8_t size)
{
	float Num;
	uint16_t u;
	...
	Num = num - (float) ((uint16_t) num);
	u = (uint16_t) ((float) Num * (float) oled_pow(10, FloatLen))
			% oled_pow(10, FloatLen);
	OLED_ShowNum(x + (size / 2), y, (uint16_t) num, IntLen, size);
	OLED_ShowChar(x + (size / 2) + IntLen * (size / 2), y, '.', size);
	OLED_ShowNum_F(x + (size / 2) + (IntLen + 1) * (size / 2), y, u, FloatLen,
			size);
}
